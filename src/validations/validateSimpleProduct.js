...
const validateProductSkus = (productData) => {
  ...
  skus.forEach((sku, index) => {
    ...
    const skuIdentifier = `SKU ${index + 1}${
    ...
    if (hasSalePrice) {
      ...
      if (!isNaN(oriPrice) && oriPrice > 0 && price >= oriPrice) {
        // <--- SỬA LẠI THÀNH >=
        allErrors.push({
          field: `${skuPrefix}.price`,
    ...
    ["width", "length", "height", "weight"].forEach((key) => {
      ...
      if (sku[key] == null || sku[key] === "") {
        ...
        allErrors.push({
          field: `${skuPrefix}.${key}`,
          message: `${name} của ${skuIdentifier} phải từ ${min} đến ${max} ${
            key === "weight" ? "g" : "cm"
          }.`,
        });
      }
    ...
    if (!isNaN(l) && !isNaN(w) && !isNaN(h) && !isNaN(wt)) {
      sku.volumetricWeight = calculateVolumetricWeight(l, w, h);
      sku.chargeWeight = calculateChargeableWeight(wt, l, w, h);
    }
    ...
    if (Array.isArray(sku.mediaUrls)) {
      sku.mediaUrls.forEach((media) => {
        ...
        if (typeof media === "object" && media.file instanceof File) {
          ext = media.file.name.split(".").pop().toLowerCase();
...
const validateSimpleProduct = async (req, res, next) => {
  ...
  const errors = [];
  const { name, thumbnail, hasVariants, categoryId, brandId, orderIndex } =
    product;
  ...
  const uniqueErrors = errors.filter(
    (e, i, self) =>
      i ===
      self.findIndex((x) => x.field === e.field && x.message === e.message)
  );
