    ...
    async createMany(req, res) {
      try {
        ...
        if (!notificationId || !Array.isArray(userIds) || userIds.length === 0) {
          return res.status(400).json({ message: 'Thiếu notificationId hoặc danh sách userIds' });
        }
        ...
        return res.status(201).json({ message: 'Gửi thông báo đến người dùng thành công' });
      } catch (err) {
  ...
  console.error('❌ Lỗi lấy danh sách user:', err.message, err.stack);
    ...
    async deleteByNotification(req, res) {
      try {
        ...
        return res.json({ message: `Đã xoá ${deleted} bản ghi notification-user.` });
      } catch (err) {
