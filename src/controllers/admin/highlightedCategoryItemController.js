...
static async create(req, res) {
  ...
  try {
    ...
    await t.rollback();
    console.error(err);
    if (err.status === 400) return res.status(400).json({ errors: err.errors });
 ...
 static async list(req, res) {
  try {
    const {
      ...
      page = 1,
      limit = 10,
      status = 'all'     // thêm param status
    } = req.query;
    const offset = (page - 1) * limit;

    // build điều kiện tìm kiếm + filter status
    const whereClause = {};
    if (search) {
    ...
    const { rows, count } = await HighlightedCategoryItem.findAndCountAll({
      ...
      include: {
        ...
        as: 'category',
        attributes: ['id', 'name'],
        // chỉ include category đang hoạt động, chưa xóa, isDefault = 0
        where: {
          isActive    : true,
          isDefault   : false,
          deletedAt   : null
        },
        required: false  // nếu muốn vẫn lấy items ngay cả khi category không thỏa
      },
      offset: +offset,
      limit:  +limit,
      order:  [['sortOrder', 'ASC']]
   ...
   static async update(req, res) {
    ...
    try {
      const { id } = req.params;
      const item = await HighlightedCategoryItem.findByPk(id, { transaction: t });
      ...
      if (req.body.customTitle && req.body.customTitle!==item.customTitle) {
        ...
        const existing = await HighlightedCategoryItem.findAll({
          ...
          attributes:['slug'], transaction: t
        });
        let slug = baseSlug;
        if (existing.length) {
          const suffix = existing
            .map(it=>{ const m=it.slug.match(new RegExp(`^${baseSlug}-(\\d+)$`)); return m?+m[1]:null; })
            .filter(n=>n!=null);
          slug = suffix.length?`${baseSlug}-${Math.max(...suffix)+1}`: `${baseSlug}-1`;
        }
        req.body.slug = slug;
      }
      ...
      if (req.body.sortOrder && +req.body.sortOrder !== item.sortOrder) {
        ...
        await HighlightedCategoryItem.update(
          { sortOrder: literal('sortOrder + 1') },
          { where:{ sortOrder:{ [Op.gte]: newOrder }, id:{ [Op.ne]: id } }, transaction: t }
        );
      }
