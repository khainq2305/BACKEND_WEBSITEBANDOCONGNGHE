...
exports.searchByName = async (req, res) => {
  try {
    ...
    const matchedProducts = await Product.findAll({
      where: {
        name: { [Op.like]: `%${keyword}%` }, // không phân biệt hoa thường nếu CSDL MySQL dùng collation utf8_general_ci
        isActive: true,
      },
      attributes: [
        "id",
    ...
    // --- BẮT ĐẦU: ĐỊNH DẠNG DỮ LIỆU ĐỂ GIỐNG HỆT searchByImage ---
    const formattedResults = matchedProducts.map((product) => {
      ...
      return {
        ...
        image: primary.ProductMedia?.[0]?.mediaUrl || product.thumbnail,
        badgeImage: product.badgeImage,
        price: finalPrice, // Sử dụng 'price'
        oldPrice: finalOldPrice, // Sử dụng 'oldPrice'
        originalPrice: primary.originalPrice, // Sử dụng 'originalPrice'
        discount: calculatedDiscount,
        skus: skus,
        inStock: totalStock > 0,
        similarity: null, // <-- thêm dòng này
        // Không có `similarity` ở đây vì đây là tìm kiếm văn bản.
      };
    });

    // Trả về danh sách sản phẩm dưới key `similarProducts` để khớp với searchByImage
    return res.status(200).json({ similarProducts: formattedResults });
    // --- KẾT THÚC: ĐỊNH DẠNG DỮ LIỆU ĐỂ GIỐNG HỆT searchByImage ---
  } catch (err) {
...
exports.getSuggestions = async (req, res) => {
  ...
  try {
      ...
      const products = await Product.findAll({
          ...
          order: [ // Sắp xếp để gợi ý tốt hơn
            [Sequelize.literal(`CASE WHEN name LIKE '${lowerCaseQuery}%' THEN 0 ELSE 1 END`), 'ASC'], // Ưu tiên tên bắt đầu bằng query
      ...
      activeCatDeals.forEach((d) => {
        const stored = catDealMap.get(d.categoryId);
        if (!stored || d.priority > stored.priority) { // Chọn deal có ưu tiên cao hơn
          catDealMap.set(d.categoryId, {
            discountType: d.discountType,
            discountValue: d.discountValue,
            priority: d.priority,
      ...
      products.forEach(product => {
          ...
          if (!uniqueSuggestionsMap.has(formattedResult.name.toLowerCase())) {
              uniqueSuggestionsMap.set(formattedResult.name.toLowerCase(), formattedResult);
          }
      });
  } catch (error) {
...
exports.getSearchHistory = async (req, res) => {
  ...
  if (!userId) { // Nếu không có userId (người dùng chưa đăng nhập hoặc token không hợp lệ)
    return res.status(401).json({ message: "Vui lòng đăng nhập để xem lịch sử tìm kiếm." });
  }
  ...
  try {
    ...
    return res.status(500).json({ message: "Lỗi server khi lấy lịch sử tìm kiếm." });
  }
};
...
exports.addSearchHistory = async (req, res) => {
  ...
  if (!userId) { // Nếu không có userId, không được phép thêm vào lịch sử
    return res.status(401).json({ message: "Vui lòng đăng nhập để thêm vào lịch sử tìm kiếm." });
  }
  ...
  try {
    ...
    return res.status(500).json({ message: "Lỗi server khi thêm vào lịch sử tìm kiếm." });
  }
};
...
exports.deleteSearchHistoryItem = async (req, res) => {
  ...
  if (!userId) { // Nếu không có userId, không được phép xóa
    return res.status(401).json({ message: "Vui lòng đăng nhập để xóa lịch sử tìm kiếm." });
  }
  ...
  try {
    ...
    if (deletedRows > 0) {
      ...
      return res.status(404).json({ message: "Không tìm thấy mục lịch sử hoặc không có quyền xóa." });
    }
  } catch (error) {
    console.error("❌ Lỗi khi xóa mục lịch sử tìm kiếm:", error);
    return res.status(500).json({ message: "Lỗi server khi xóa mục lịch sử tìm kiếm." });
  }
};