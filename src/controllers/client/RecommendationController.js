...
class RecommendationController {
  ...
  static async _getUserPurchasedProducts(userId, limit = 5) {
    ...
    try {
      ...
      const products = Array.isArray(queryResults)
        ? queryResults
        : queryResults
        ? [queryResults]
        : [];
  ...
  // Lấy category/brand từ sản phẩm hiện tại
  if (currentProductId) {
    const currentProduct = await Product.findByPk(currentProductId, {
      attributes: ["categoryId", "brandId", "isActive"],
      where: { isActive: true }, // chú ý: findByPk không hỗ trợ where, nếu cần thì phải dùng findOne
    });
    if (currentProduct) {
  ...
  if (relatedCategoryIds.size > 0) {
    whereClause[Op.or].push({
      categoryId: { [Op.in]: Array.from(relatedCategoryIds) },
    });
  }
  if (relatedBrandIds.size > 0) {
    whereClause[Op.or].push({
      brandId: { [Op.in]: Array.from(relatedBrandIds) },
    });
  }

  // Nếu có liên quan category/brand thì query trước
      ...
      order: [
        [
          sequelize.literal(
            ...
            "WHERE `oi`.`skuId` IN (SELECT `id` FROM `skus` WHERE `skus`.`productId` = `products`.`id`))"
          ),
          "DESC",
        ],
      ],
  ...
  static async _buildGeminiRecommendationPrompt(
    userId,
    currentProductId = null
  ) {
    let prompt =
      "Bạn là một chuyên gia gợi ý sản phẩm cho một trang thương mại điện tử. " +
    ...
    const recentlyViewed =
      await RecommendationController._getUserRecentlyViewedProducts(userId, 5);
    const purchasedProducts =
      await RecommendationController._getUserPurchasedProducts(userId, 5);
    const searchHistory = await RecommendationController._getUserSearchHistory(
      userId,
      5
    );
    const userDemographics =
      await RecommendationController._getUserDemographics(userId);
    const currentProduct = currentProductId
      ? await RecommendationController._getProductDetailsForGemini(
          currentProductId
        )
      : null;
    ...
    if (currentProduct) {
      prompt += `\n\nNgười dùng đang xem: "${currentProduct.name}" (Danh mục: ${
        currentProduct.category?.name || "không rõ"
      }, Thương hiệu: ${currentProduct.brand?.name || "không rõ"}).`;
    }
    ...
    if (purchasedProducts.length > 0) {
      const productNames = purchasedProducts
        .map((p) => `"${p.name}"`)
        .join(", ");
    ...
    const productsForPrompt =
      await RecommendationController._getRelatedProductsForPrompt(
        userId,
        currentProductId
      );
  ...
  static async _getPopularProducts(limit = 10) {
    try {
      ...
      const topRows = await Product.findAll({
        attributes: [
          ...
          [
            sequelize.fn(
              "COALESCE",
              sequelize.fn("SUM", sequelize.col("skus.OrderItems.quantity")),
              0
            ),
            "soldCount",
          ],
      ...
      const products = await Product.findAll({
        ...
        order: [
          [
            sequelize.literal(`FIELD(\`Product\`.\`id\`, ${ids.join(",")})`),
            "ASC",
          ],
        ],
        limit: ids.length,
  ...
  static async _getGeminiRecommendations(userId, currentProductId = null) {
    ...
    try {
      const promptText =
        await RecommendationController._buildGeminiRecommendationPrompt(
          userId,
          currentProductId
        );
    ...
    if (recommendedProductIds.length > 0) {
      try {
        ...
        const mappedProducts = new Map(
          recommendedProductsFromDb.map((p) => [p.id, p])
        );
    ...
    try {
      formattedRecommendations = await this._mapProductsToFrontendFormat(
    ...
    try {
      const recentlyViewedForExclusion =
        await RecommendationController._getUserRecentlyViewedProducts(
          userId,
          10
        );
  ...
  static async _mapProductsToFrontendFormat(products) {
    ...
    return products.map((product) => {
      ...
      const processedSkus = (productJson.skus || [])
        .map((sku) => {
      ...
      (productJson.skus || []).forEach((sku) => {
        if (sku.OrderItems) {
          totalSoldCount += sku.OrderItems.reduce(
