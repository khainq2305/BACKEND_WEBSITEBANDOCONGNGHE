    ...
    const data = sections.map((sec) => {
      ...
      if (noBannerTypes.includes(typeNorm)) {
        if (section.banners?.length) {
          console.log(`[getHomeSections] strip banners for section id=${section.id} type=${section.type}`);
        }
        section.banners = [];
      }
      if (typeNorm === 'onlyProduct') {
        if (section.linkedCategories?.length) {
          console.log(`[getHomeSections] strip linkedCategories for section id=${section.id} type=${section.type}`);
        }
      ...
      section.products = (section.products || []).map((prod) => {
        ...
        return {
          ...prodData,
          skus: processedSkus,
          defaultSku: selectedDefaultSku || null,
          soldCount,
          rating,
