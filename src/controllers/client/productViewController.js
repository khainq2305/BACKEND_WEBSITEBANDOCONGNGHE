  ...
  static async getRecentlyViewedByCategoryLevel1(req, res) {
    try {
      if (!req.user || !req.user.id) {
        return res
          .status(403)
          .json({ message: "Cần đăng nhập để lấy sản phẩm đã xem gần đây." });
      }
  ...
  static async searchForCompare(req, res) {
    try {
            ...
            include: [
              {
                model: Sku,
                as: "sku",
                attributes: ["id", "productId", "price", "originalPrice"],
              },
            ],
          },
          {
        ...
        (saleEvent.flashSaleItems || []).forEach((fsi) => {
          ...
          if (!isSoldOut && flashItemSalePrice > 0) {
            if (
              ...
              flashItemSalePrice <
                allActiveFlashSaleItemsMap.get(sku.id).salePrice
            ) {
      ...
      const productsWithPrices = rows.map((product) => {
        ...
        const topLevelCategoryId =
          pj.category?.parentCategory?.id || pj.category?.id;
        const topLevelCategoryName =
          pj.category?.parentCategory?.name || pj.category?.name;
