const { 
  Product, Category, Brand, Sku, ProductMedia, 
  SkuVariantValue, VariantValue, Variant, 
  ProductInfo, ProductSpec, Review, OrderItem,
  FlashSaleItem, FlashSale // ✅ THÊM 2 MODEL NÀY VÀO
} = require('../../models');
...
static async getProductsByCategory(req, res) {
  try {
    ...
    /********** 5. LẤY CHI TIẾT SKU + MEDIA CHO PAGE HIỆN TẠI **********/
          ...
          include: [
            {
              ...
              attributes: ["mediaUrl"],
              separate: true,
              limit: 1,             // chỉ lấy ảnh đầu cho card
            },
          ],
        },
      ],
      // giữ đúng thứ tự của query gốc
      order: [Sequelize.literal(`FIELD(Product.id, ${pageIds.join(",")})`)],
    });

    /********** 6. GOM DỮ LIỆU & TÍNH TRƯỜNG HIỂN THỊ **********/
    const formatted = details.map((p) => {
      // sku ánh xạ đầu tiên (giá thấp nhất – nhờ MIN trong query)
      ...
      if (price && price > 0) {
        ...
        displayPrice = original;   // chỉ có giá gốc
      }

      const agg = rows.find((r) => r.id === p.id); // record đã GROUP
      return {
        id: p.id,
        name: p.name,
        slug: p.slug,
        ...
        skus: p.skus,              // gửi kèm array sku (nếu client cần chi tiết)
      };
    });
    ...
    const formattedProducts = products.map(p => {
      ...
      if (productJson.skus?.length > 0) {
        ...
        if (validSkus.length > 0) {
          const bestSku = validSkus.reduce((min, s) => s.price < min.price ? s : min, validSkus[0]);
          finalPrice = bestSku.price;
          if (bestSku.originalPrice && bestSku.originalPrice > bestSku.price) {
      ...
      if (finalOldPrice && finalPrice) {
        discount = Math.round(((finalOldPrice - finalPrice) / finalOldPrice) * 100);
      }
