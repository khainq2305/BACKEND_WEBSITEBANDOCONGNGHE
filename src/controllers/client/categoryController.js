const { Category, categoryPostModel } = require('../../models'); // <-- Sửa PostCategory thành categoryPostModel
    ...
    try {
      const [productCats, postCats] = await Promise.all([
        ...
        categoryPostModel.findAll({ 
                    attributes: ['id', 'name', 'slug', 'parentId', 'isActive', 'sortOrder'],
                    where: { isActive: 1, deletedAt: null },
                    order: [['sortOrder', 'ASC']],
                }),
      ]);
      ...
      const productTree = productParents.map((parent) => {
        const sub = productChildren.filter((child) => child.parentId === parent.id);
        return {
      ...
      return res.status(500).json({ message: 'Lỗi server khi lấy danh mục tổng hợp' });
    }
  }
