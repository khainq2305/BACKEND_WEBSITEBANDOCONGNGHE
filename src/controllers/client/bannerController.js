...
class bannerController {
  ...
  static async getByType(req, res) {
    try {
      ...
      return res.status(500).json({ message: 'Lỗi lấy banner phía client', error: error.message });
    }
  }
   static async getCategoryBanner(req, res) {
    try {
      ...
      return res.status(500).json({ message: 'Lỗi lấy banner danh mục', error: error.message });
    }
  }
static async getProductBanner(req, res) {
  try {
    ...
    return res.status(500).json({ message: 'Lỗi lấy banner sản phẩm', error: error.message });
  }
}
